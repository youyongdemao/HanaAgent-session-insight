...
- **计费规则外置化**：单价、峰谷时段、模型归属、上下文窗口从代码里抽到仓库根目录的 `pricing.json`，插件每天自动拉取一次（jsDelivr 优先、GitHub raw 兜底），内置快照作最后兜底
- **配置变化自动感知**：检测到宿主 `provider-catalog.json` / `models.json` 变化时自动重算，新增供应商无需重启
- **费用简报详情新增「重新加载配置」**：改完配置点一下就生效，不用等轮询周期
- **数据库全量、使用按需**：已知供应商与模型全部收录，界面按你实际配置的供应商过滤展示；缺价格的模型照样统计 Token / 轮数 / 命中率，只把费用标为「价格未收录」
- **修复插件接口路径**：`pluginApiFetch` 系列调用补上 `/api` 前缀，余额刷新、规则保存与重新加载配置恢复正常
...
因此 **v1.2.x 的老版本可以直接通过「检查更新」一键升级到 v2.1.0**，无需手动重装。
