...
- 修复卡片资源缓存导致前端更新不生效的问题
- 用量页左右两列卡片底边对齐，区块间距恢复正常
- 模型用量表补上表头与 Token 占比条，数字列右对齐
- 日志卡片支持近 2h / 近 24h 切换，高度收敛

## v2.0.3 更新要点

- **自动检查更新**：启动、打开页面与手动刷新时查新，有新版弹窗确认更新
- **用量数据源跟随宿主**：改读 SQLite，总消耗、今日用量与调用数恢复准确
- **首屏秒开**：会话与账本先上屏，余额和计费库后台补齐
- **价格列表只显示已配置的模型**
- **数字转轮**：从当前值续滚，加缓启缓停
- **侧栏状态条**：窄宽度下不再折行；用量状态栏改为沉浸式
- **修复**：今日输入构成数值对齐、安装与「重新加载配置」在部分宿主上报错
