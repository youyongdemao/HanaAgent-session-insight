...
- `v2.0.x`：推送远端 `origin/v2.0.x`，release 从这条线出。
- `v2.1.x`：只本地提交，不 push、不打 tag、不发 release。要发行必须等猫猫明确开口。
- 远端 `main` 指向 2.1.0 架构的历史提交，仅作历史保留，不代表发行线；仓库默认分支应指向 `v2.0.x`。
- 远端 tag `v2.1.0` 存在但不代表已发行，GitHub Releases 里没有对应的 release。
